...
object_detected_4 = "red"
object_detected_5 = "blue"
threshold = 0.2
max_detect_object = 1
...
# outdoor mission
outdoor_drop_mission_position_1_forward_waypoint = 13
outdoor_drop_mission_position_1_loiter_waypoint = 14
outdoor_drop_mission_position_2_forward_waypoint = 15
outdoor_drop_mission_position_2_loiter_waypoint = 16
going_new_land_loiter_waypoint = 17
going_new_land_forward_waypoint = 18
new_land_loiter_waypoint_left = 19
new_land_loiter_waypoint_right = 20
...
connection_string_vehicle = '/dev/ttyAMA0, 115200'
# connection_string_vehicle = '/dev/ttyAMA1, 115200'
# connection_string_vehicle = '/dev/ttyAMA2, 115200'
# connection_string_vehicle = '/dev/ttyAMA3, 115200'
# connection_string_vehicle = '/dev/ttyAMA4, 115200'

# OpenGrab EPM v3 Magnetic Gripper
# define the Raspberry Pi's pin for the magnetic gripper
magnetic_gripper_pin = 13 # pin 23
raspi_gpio_pwm_on_freq = 500 # frequency in Hertz
raspi_gpio_pwm_off_freq = 1000 # frequency in Hertz
raspi_gpio_pwm_duty_cycle = 100 # PWM duty cycle (0-100) in percents
GPIO.setup(magnetic_gripper_pin, GPIO.OUT)

magnetic_gripper_pickup = GPIO.PWM(magnetic_gripper_pin, raspi_gpio_pwm_on_freq) # activate the PWM output to turn on the magnetic gripper
magnetic_gripper_release = GPIO.PWM(magnetic_gripper_pin, raspi_gpio_pwm_off_freq) # activate the PWM output to turn off the magnetic gripper
...
servo_kit = ServoKit(channels=8)
# define the servo channel on the driver
servo_gripper_left_channel = 0
...
def outdoor_payload_first():
    ...
    vehicle.commands.next = outdoor_drop_mission_position_1_forward_waypoint # going forward to outdoor first payload drop position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")

    vehicle.commands.next = outdoor_drop_mission_position_1_loiter_waypoint # arrived in outdoor first payload drop position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")
...
def outdoor_payload_second():
    ...
    vehicle.commands.next = outdoor_drop_mission_position_2_forward_waypoint # going forward to outdoor second payload drop position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")

    vehicle.commands.next = outdoor_drop_mission_position_2_loiter_waypoint # arrived in outdoor second payload drop position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")
...
def going_to_new_land_position_mission():
    ...
    vehicle.commands.next = going_new_land_forward_waypoint # going forward to new landing zone position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")

    vehicle.commands.next = going_new_land_loiter_waypoint # going forward to new landing zone position
    print("\n")
    print("[INFO] Current Waypoint: ", vehicle.commands.next)
    print("\n")
...
cap_bottom.set(cv2.CAP_PROP_FPS, 10)
...
cap_front.set(cv2.CAP_PROP_FPS, 10)
...
def main_code():
    ...
    print("====================================================================================================================================================")
    print("\n")
    print("[INFO] MISSION IS STARTING...")
    print("\n")
    print("====================================================================================================================================================")
    ...
    if vehicle.location.global_relative_frame.alt == altitude_indoor:
        vehicle_altitude()
        vehicle.mode = VehicleMode("LOITER")
        print("[INFO] Vehicle in LOITER Mode")
        print("\n")
        vehicle_altitude()
    if vehicle.location.global_relative_frame.alt < altitude_indoor:
        vehicle_altitude()
        vehicle.mode = VehicleMode("GUIDED")
        send_attitude_target(yaw_angle= start_heading, thrust=0.7)
        vehicle_altitude()
        if vehicle.location.global_relative_frame.alt == altitude_indoor:
          vehicle_altitude()
          vehicle.mode = VehicleMode("LOITER")
    ...
    if vehicle.location.global_relative_frame.alt > altitude_indoor:
        vehicle_altitude()
        vehicle.mode = VehicleMode("GUIDED")
        send_attitude_target(yaw_angle= start_heading, thrust=0.3)
        vehicle_altitude()
        if vehicle.location.global_relative_frame.alt == altitude_indoor:
          vehicle_altitude()
          vehicle.mode = VehicleMode("LOITER")
    ...
    magnetic_gripper_pickup.start(raspi_gpio_pwm_duty_cycle)

    # loop over some frames...this time using the threaded stream
    while fps._numFrames < args["num_frames"]:

        # Bottom Camera
        frame_bottom = cap_bottom.imread()
        ...
        print("\n")
        print("[INFO] Distances from Vehicle to the Front Object : ", round(distances_milimeters*100, 2))
        print("\n")
        ...
        if result_df_bottom.empty == False or result_df_front.empty == False and object_bottom == object_detected_1 or object_front == object_detected_1 :
            print("\n")
            ...
            if result_df_bottom.empty == False or result_df_front.empty == False and object_bottom == object_detected_2 or object_front == object_detected_2 :
              print("\n")
              ...
              magnetic_gripper_release.start(raspi_gpio_pwm_duty_cycle)

              front_cam_servo_forward()

              window_pass_mission_before()

              if result_df_front == False and object_front == object_detected_3:
                print("\n")
                print("[INFO] Detection result: ", object_detected_1, "detected")
                print("\n")
                ...
                # if yaw_angle_option == -1:
                #   if color_prediction == object_detected_4:
                ...
                # if yaw_angle_option == 1:
                #   if color_prediction == object_detected_5:
...
# wait_key() # wait for the key press to continue
main_code()
